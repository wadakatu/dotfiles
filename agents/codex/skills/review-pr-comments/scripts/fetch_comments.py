...
Usage:
  python fetch_comments.py [PR_NUMBER_OR_URL] > pr_comments.json
"""
...
def _ensure_gh_authenticated() -> None:
    try:
        ...
        raise RuntimeError(
            "gh auth status failed; fix an invalid GITHUB_TOKEN or run `gh auth login` to authenticate the GitHub CLI"
        ) from error


def gh_pr_view_json(fields: str, target: str | None = None) -> dict[str, Any]:
    cmd = ["gh", "pr", "view"]
    if target:
        cmd.append(target)
    cmd += ["--json", fields]
...
def resolve_pr_ref(target: str | None = None) -> tuple[str, str, int]:
    """Resolve the base repository and PR number from gh's canonical PR URL."""
    pr = gh_pr_view_json("number,url", target)
    parts = pr["url"].removeprefix("https://github.com/").split("/")
...
def main() -> None:
    if len(sys.argv) > 2:
        raise RuntimeError("Usage: fetch_comments.py [PR_NUMBER_OR_URL]")
    _ensure_gh_authenticated()
    target = sys.argv[1] if len(sys.argv) == 2 else None
    owner, repo, number = resolve_pr_ref(target)
